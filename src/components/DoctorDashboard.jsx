...
export default function DoctorDashboard() {
  ...
  const openPatient = async (p) => {
    try {
      const record = await api.encounter(p.id)
      setSelectedPatient({
        ...p,
        id: record.patient?.id || p.id,
        name: record.patient?.name || p.name,
        gender: record.patient?.gender || p.gender,
        encounterRecord: record,
      })
    } catch (error) {
      console.warn('Record API unavailable; opening demo record.', error)
